use api_v1::models::AdUpdateObject;
use common::mkpl_ad::*;

use crate::mappers::base::*;

#[derive(Debug)]
pub struct AdUpdateObjectMapper;

impl AdUpdateObjectMapper {
    pub fn from_api(value: &Option<AdUpdateObject>) -> MkplAd {
        match value {
            Some(api_obj) => MkplAd {
    ...
    pub fn to_api(mkpl_ad: &MkplAd) -> Option<AdUpdateObject> {
        Some(AdUpdateObject {
...
mod tests {
    ...
    fn test_full_conversion_cycle() {
        ...
        let api = AdUpdateObjectMapper::to_api(&mkpl).unwrap();
        ...
        let converted_back = AdUpdateObjectMapper::from_api(&Some(api));
